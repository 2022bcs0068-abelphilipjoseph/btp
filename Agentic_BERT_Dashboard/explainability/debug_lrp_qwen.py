...
def clean_token(token):
    # Qwen tokens might have different artifacts
...
def debug_qwen_explanations():
    print("--- Explaining Qwen (Finetuned) ---")
    ...
    class ModelWrapper(torch.nn.Module):
        ...
        def forward(self, input_embeds):
            return self.model(inputs_embeds=input_embeds).logits[:, -1, :]

    wrapper = ModelWrapper(model)
    
    # Access embedding layer just to get embeddings
    try:
    ...
    # Embeddings: Use IntegratedGradients on INPUT EMBEDDINGS directly
    # This attributes to the inputs of the wrapper (which are embeddings)
    ig = IntegratedGradients(wrapper)
    try:
        # We attribute to the INPUT EMBEDDINGS tensor
        ...
        layer_attributions.append(np.zeros(inputs_embeds.shape[1]))
    
    # Layers: Use LIG (Sample layers if too many)
    stride = 1
    if len(layers) > 12:
        stride = 2 # Visualization optimization
    ...
    lrp_heatmap = visualize_heatmap(layer_attrs_np, cleaned_tokens, layer_names, 
                                    title="Layer-wise Relevance (LIG) - Qwen",
    ...
    try:
        ...
        # Reduce max evals to avoid timeout
        ...
        shap_html = "<p>SHAP calculation failed (memory/complexity).</p>"
    ...
    with torch.no_grad():
        ...
        prob_orig = torch.softmax(out_orig.logits[0, -1], dim=0)[target_token_id].item()
        
        out_cf = model(**inputs_cf)
        prob_cf = torch.softmax(out_cf.logits[0, -1], dim=0)[target_token_id].item()
    ...
    <div class='container' style='background-color: #f0f8ff; padding: 20px; border-radius: 10px; margin-bottom: 20px;'>
        ...
        <h3>1. Layer-wise Integrated Gradients (Approximating LRP)</h3>
        {lrp_heatmap}
